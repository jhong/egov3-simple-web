 ...
 * by jhong (2016.03.24)
 */
$.validator.addMethod("required_by", function(value, element, options) {
	var formName = element.form.name;
	var firstElmtName = options[0]; // ex. qty
	var secondElmtName = element.name; // ex. qtyunit
	
	var firstObj = getFormObject(formName, firstElmtName);
	var secondObj = getFormObject(element.form.name, secondElmtName);
	//showlog("required_by() value="+value+", formName="+formName+", "+firstElmtName+", "+secondElmtName+", "+firstObj.value+", "+secondObj.value);
...
/**
 * 공백만 입력한 경우 필수입력 체크 필요한 경우 사용
 * (양 끝 공백 trim 후 필수입력 체크)
 * by jhong (2016.05.18)
 */
$.validator.addMethod("requiredWords", function(value, element) {
